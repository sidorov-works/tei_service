...
# Hugging Face tokenizers по умолчанию включает многопоточность на уровне C++ 
# для ускорения токенизации. Однако при использовании multiprocessing (через fork)
# это может привести к deadlock'ам, так как дочерние процессы наследуют 
# заблокированные мьютексы из родительского процесса.
#
# ПОЧЕМУ ЭТО ВАЖНО ДЛЯ НАШЕГО СЕРВИСА:
# 1. Мы используем asyncio.to_thread() для вызова модели в отдельном потоке
# 2. При создании новых потоков может происходить fork процесса
# 3. Tokenizers, загруженный до установки этой переменной, уже создал бы
#    свои внутренние потоки, что потенциально опасно
#
# ЧТО ДЕЛАЕТ ЭТА УСТАНОВКА:
# "TOKENIZERS_PARALLELISM=false" заставляет tokenizers НЕ использовать
# внутреннюю многопоточность, а работать в последовательном режиме.
#
# ПОЧЕМУ ЭТО БЕЗОПАСНО ДЛЯ НАС:
# - Наша архитектура с очередью и одним воркером уже подразумевает
#   последовательную обработку задач
# - Мы не выиграем от многопоточности tokenizers, так как модель
#   SentenceTransformer всё равно не thread-safe и вызывается последовательно
# - Мы теряем только внутренний параллелизм tokenizers, но не влияем
#   на нашу очередь задач
#
# ВАЖНО: Эта переменная должна быть установлена ДО импорта любых модулей,
# которые могут импортировать transformers или tokenizers, так как
# библиотека читает её один раз при инициализации.
import os
