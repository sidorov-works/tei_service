...
- Длина текста ≤ `MAX_TEXT_LENGTH` (по умолчанию 10000 символов)

##### BatchEncodeRequest

```python
{
    "texts": ["string1", "string2"],   # обязательное поле
    "request_type": "query"             # "query" или "document", опционально
}
```
...
- Количество текстов ≤ `MAX_BATCH_SIZE` (по умолчанию 256)
- Каждый текст не пустой и ≤ `MAX_TEXT_LENGTH`
- Суммарная длина всех текстов ≤ `MAX_TOTAL_BATCH_LENGTH` (по умолчанию 100000 символов)
...
## Подпись запросов (JWT аутентификация)

Все рабочие эндпоинты (`/encode`, `/encode_batch`, `/count_tokens`, `/count_tokens_batch`) 
защищены JWT аутентификацией. Клиент **обязан** предоставить валидный JWT токен 
в каждом запросе.

### 🔑 Формат токена

Токен должен быть подписан алгоритмом **HS256** с использованием общего секретного ключа 
(`INTERNAL_API_SECRET`), одинакового для всех сервисов в инфраструктуре.

**Структура payload:**
```json
{
    "iss": "service-name",        // обязательное поле - имя сервиса-отправителя
    "iat": 1700000000,            // время выпуска (Unix timestamp)
    "exp": 1700000030,            // время истечения (iat + 30 секунд)
    "service": "service-name",     // дублирование имени для удобства
    // дополнительные поля по желанию (request_id, user_id и т.д.)
}
```

**Важные требования:**
- **Короткое время жизни** - 30 секунд (защита от replay-атак)
- **Обязательные поля** - `iss` и `exp` должны присутствовать
- **Часы должны быть синхронизированы** (используйте NTP)

### 📤 Передача токена

Токен передается в HTTP-заголовке стандартным способом:
```
Authorization: Bearer <jwt-token>
```

### 💻 Примеры создания токена на разных языках

**Python (с использованием PyJWT):**
```python
import jwt
import time
...
payload = {
    "iss": service_name,
    "iat": int(time.time()),
    "exp": int(time.time()) + 30,  # 30 секунд жизни
    "service": service_name,
    "request_id": "req-123"  # опционально, для трекинга
}
...
Сервис возвращает HTTP статусы:

| Код | Описание | Пример ответа |
|-----|----------|---------------|
| **403** | Токен отсутствует | `{"detail": "Missing authentication token"}` |
| **403** | Токен истек | `{"detail": "Invalid token: Signature has expired"}` |
| **403** | Неверная подпись | `{"detail": "Invalid token: Signature verification failed"}` |
| **403** | Неверный формат | `{"detail": "Invalid token: Not enough segments"}` |

### 🔒 Рекомендации по безопасности

1. **Никогда не включайте JWT в URL** - только в заголовок Authorization
2. **Храните секретный ключ безопасно** - используйте переменные окружения, Vault, K8s secrets
3. **Меняйте секретный ключ при компрометации** - во всех сервисах одновременно
4. **Используйте HTTPS** в production для шифрования трафика
5. **Синхронизируйте время** на всех серверах через NTP

### 🔄 Автоматическое обновление токенов

Для длительных операций (например, пакетная индексация) клиент должен:
1. Создавать новый токен **для каждого запроса**
2. Не использовать один токен для нескольких запросов
3. При получении 403 с истечением - повторить запрос с новым токеном

Пример стратегии (псевдокод):
```python
async def make_authenticated_request(url, data):
    # Свежий токен на каждый запрос
    token = create_token(service_name)
    headers = {"Authorization": f"Bearer {token}"}
    
    response = await http_client.post(url, headers=headers, json=data)
    
    if response.status == 403 and "expired" in response.text:
        # Токен истек (маловероятно при 30 сек, но обработаем)
        token = create_token(service_name)
        headers["Authorization"] = f"Bearer {token}"
        response = await http_client.post(url, headers=headers, json=data)
...
- [ ] Получили `INTERNAL_API_SECRET` (минимум 32 символа)
- [ ] Настроили синхронизацию времени (NTP)
- [ ] Реализовали генерацию JWT с expiration 30 сек
- [ ] Добавили заголовок Authorization во все POST запросы
- [ ] Обрабатываем 403 ошибки (особенно expired)
- [ ] Не логируем и не сохраняем токены
...
def create_token(service_name="test-client"):
    payload = {
        "iss": service_name,
        "iat": int(time.time()),
...
| **403 Token expired** | Синхронизируйте время на серверах (NTP). Время жизни токена жестко задано 30 секундами |
